...
pcs resource delete cbdb-vip
pcs resource delete cbdb
...
#
pcs resource create cbdb-vip ocf:heartbeat:IPaddr2 ip=192.168.0.180 cidr_netmask=24 op monitor interval=30s
...
pcs constraint colocation add cbdb-vip gpcc INFINITY

# Set the 'virtual_ip' and 'webserver' resources always on same node servers.
pcs constraint order cbdb-vip then gpcc

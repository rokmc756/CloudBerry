...
pcs resource create cbdb-vip ocf:heartbeat:IPaddr2 ip=192.168.0.180 cidr_netmask=24 nic=eth0 iflabel=0 op monitor interval=60s \
on-fail=standby

pcs resource create cbdb ocf:heartbeat:cbdb op monitor interval=60s \
on-fail=standby
# pcs resource create cbdb ocf:heartbeat:cbdb op monitor timeout=30s interval=10s on-fail=standby role="Promoted" promotable

pcs constraint location cbdb prefers rk8-master=50
pcs constraint location cbdb-vip prefers rk8-master=50

# pcs constraint colocation add cbdb with cbdb-vip INFINITY
pcs constraint colocation add cbdb with cbdb-vip INFINITY

# Set the 'gpcc-vip' and 'gpccr' resources always on same node servers.
# pcs constraint order cbdb then cbdb-vip
pcs constraint order cbdb then cbdb-vip

# pcs resource meta cbdb migration-threshold=4
# pcs resource meta cbdb-vip migration-threshold=4

pcs resource meta cbdb resource-stickiness=0
pcs resource meta cbdb-vip resource-stickiness=0

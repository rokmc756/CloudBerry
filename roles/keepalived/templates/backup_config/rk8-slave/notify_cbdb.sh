...
case $STATE in
        "MASTER") # su gpadmin -c "mv /data/master/gpseg-1/recovery.conf /data/master/gpseg-1/recovery.done"
                  su gpadmin -c "source /usr/local/cloudberry-db/greenplum_path.sh && export PGPORT=5432 && gpactivatestandby -a -f -d /data/master/gpseg-1"
                  su gpadmin -c "source /usr/local/cloudberry-db/greenplum_path.sh && gpinitstandby -a -s rk8-master"
                  exit 0
                  ;;
        "BACKUP") su gpadmin -c "mv /data/master/gpseg-1/recovery.done /data/master/gpseg-1/recovery.conf"
                  ...
                  exit 0
                  ;;
        "FAULT") su gpadmin -c "source /usr/local/cloudberry-db/greenplum_path.sh && echo test"
                  exit 0
                  ;;
        *)        echo "Unknown state"
                  exit 1
